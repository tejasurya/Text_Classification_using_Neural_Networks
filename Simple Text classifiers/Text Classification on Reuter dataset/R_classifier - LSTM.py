...
model.add(LSTM(gru_output_size, return_sequences=True, dropout=0.2, recurrent_dropout=0.2))
model.add(Flatten())
...
print(model.summary())
# fit the model
model.fit(padded_docs,y_train, nb_epoch=5, validation_data=(vpadded_docs, y_valid), callbacks=[metriczs])

# evaluate the model
loss, accuracy = model.evaluate(tpadded_docs, y_test)
