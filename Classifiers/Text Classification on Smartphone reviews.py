...
os.chdir("G:/NLP/Dataset/amazon_mob");
df=pd.read_csv('amazon_mob.csv');

#input
ip=df['Reviews'].values.tolist()
ip=ip[0:sample_cnt]
...
def parallel_cross_straight_relu():
    ...
    conv_0 = Conv1D(64,kernel_size,padding='valid',strides=1, activation='relu')(e)
    maxpool_0 = MaxPooling1D(pool_size=8)(conv_0)
    gru=GRU(gru_output_size, return_sequences=True, dropout=0.2, recurrent_dropout=0.2)(e)
    maxpool_gru = MaxPooling1D(pool_size=8)(gru)
